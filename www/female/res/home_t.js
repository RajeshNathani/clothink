if(localStorage['price'] == 300){
    fetch("https://cors-anywhere.herokuapp.com/https://fashionxapi.herokuapp.com/api/women/kurtis")
    .then(res=>res.json())
    .then((out)=>{
        for(i=0;i<out.length;i++){
        if(out[i].name.includes(localStorage['color']) || parseInt(out[i].price.slice(1))<=localStorage['price']){
